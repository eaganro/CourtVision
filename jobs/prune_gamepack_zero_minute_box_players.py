...
def parse_args():
    parser = argparse.ArgumentParser(
        description="Remove zero-minute players from stored gamepack box payloads."
    )
...
def normalize_minutes(value):
    ...
    if raw.startswith("PT") and raw.endswith("S"):
        trimmed = raw[2:-1]
        if "M" in trimmed:
            mins, secs = trimmed.split("M", 1)
            try:
                return f"{int(mins):02d}:{int(float(secs or 0)):02d}"
            except ValueError:
                return "00:00"
    if ":" in raw:
...
def prune_zero_minute_players(gamepack):
    ...
    for side in ("away", "home"):
        ...
        for player in players:
            ...
            if normalize_minutes(stats.get("min")) == "00:00":
                removed += 1
                continue
            kept.append(player)
        team["players"] = kept
...
def main():
    ...
    for key in keys:
        ...
        if not isinstance(gamepack, dict):
            continue
        result = prune_zero_minute_players(gamepack)
        if not result or result["removed"] <= 0:
            continue
        changed_gamepacks += 1
        removed_players += result["removed"]
        writes.append((key, result["gamepack"], meta or {}))

    print(f"Scanned {len(keys)} target gamepack(s).")
    print(f"Would remove {removed_players} zero-minute box players across {changed_gamepacks} gamepack(s).")

    if args.dry_run:
        return 0

    for key, gamepack, meta in writes:
        write_gamepack(s3_client, args.bucket, key, gamepack, meta)

    print(f"Wrote {len(writes)} repaired gamepack(s).")
    return 0
