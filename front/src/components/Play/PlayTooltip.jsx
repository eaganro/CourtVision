  ...
  const primaryAction = (() => {
    ...
    if (focusActionMeta && focusActionMeta.actionNumber != null) {
      const focusMatch = descriptionArray.find(
        (action) =>
          focusActionMeta.actionNumber != null &&
          String(action.actionNumber) === String(focusActionMeta.actionNumber),
      );
      ...
      {(() => {
        ...
        return renderItems.map((item, index) => {
          ...
          const eventType = item.isSubSummary
            ? null
            : getEventType(a.description, a.actionType, a.result);
          ...
          const actionSide =
            a.side === 'away' || a.side === 'home'
              ? a.side
              : a.teamTricode === awayTeamNames.abr
                ? 'away'
                : 'home';
          ...
          const isOneOfOne = freeThrowOneOfOnePattern.test(
            `${a.subType || ''} ${a.description || ''}`,
          );
  ...
  return (
    <div className="descriptionArea" style={tooltipStyle} ref={tooltipRef}>
      {!shouldPositionBelow ? (
        ...
        <>
          {mobileNavControls && <div style={{ marginBottom: 6 }}>{mobileNavControls}</div>}
          <ActionsComponent />
          {lockedVideoLink}
